...
Any employees that are in departments 20, 50 and 70 display last name,  salary, and the salary if it is increased by 
5%. Make sure the output is a whole number. Also add a column that subtracts the old salary from the new salary and 
multiplies the difference by 12.
...
September 28th of year 2006  <-- this is the format for the date you display. Your result will depend on the day when 
...
/* 4) For each employee hired in 2014, display the employee’s last name, hire date and calculate the number of YEARS 
between TODAY and the date the employee was hired.  Label the column Years worked.
Order your results by the number of years employed.  Use the label to do the order by
...
that start on S and have at least 8 characters in their name. If city does not have a province name assigned, then 
...
/* 6) Display each employee’s last name, hire date, and salary review date. The salary review date is the first 
Wednesday after a year of service, but only for those hired after 2015.
